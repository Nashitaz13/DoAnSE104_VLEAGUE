...
function HomePage() {
  ...
  useEffect(() => {
    ...
    if (list.length > 0) {
      const sorted = [...list].sort((a: any, b: any) =>
        a.muagiai.localeCompare(b.muagiai),
      )
  ...
  const { data: awardsData } = useQuery({
    ...
    queryFn: () =>
      StatisticsService.getAwards({ muagiai: currentSeason, limit: 5 }),
    enabled: !!currentSeason,
  })
  ...
  const recentMatches = allMatches
    .filter((m: any) => {
      const hasScore =
        m.ketqua && typeof m.ketqua === "string" && m.ketqua.includes("-")
      const isFinishedStatus =
        m.trangthai === "DaKetThuc" || m.trangthai === "Finished"

      return isFinishedStatus || hasScore
    })
    .sort((a: any, b: any) => {
      return (
        new Date(b.ngaythi_dau).getTime() - new Date(a.ngaythi_dau).getTime()
      )
    })
    .slice(0, 3)
  ...
  const upcomingMatches = allMatches
    .filter((m: any) => {
      ...
      const isScheduled =
        m.trangthai === "SapDienRa" || m.trangthai === "Scheduled"
      return isScheduled || noScore
    })
    .sort((a: any, b: any) => {
      return (
        new Date(a.ngaythi_dau).getTime() - new Date(b.ngaythi_dau).getTime()
      )
    })
    .slice(0, 3)
  ...
  return (
    <div className="flex flex-col min-h-screen">
      {/* --- HERO BANNER --- */}
      <div className="relative text-white py-20 px-6 overflow-hidden bg-[url('/bg-stadium.jpg')] bg-cover bg-center bg-no-repeat">
        <div className="absolute inset-0 bg-red-950/80 mix-blend-multiply z-0" />
        ...
        <div className="relative container mx-auto max-w-6xl flex flex-col md:flex-row items-center justify-between gap-8 z-10">
          <div className="space-y-4 text-center md:text-left">
            <h1 className="text-4xl md:text-6xl font-extrabold tracking-tight drop-shadow-lg">
              V-League 1 <br />{" "}
              <span className="text-red-300">{currentSeason}</span>
            </h1>
            <p className="text-red-100 max-w-lg text-lg drop-shadow-md">
              Giải đấu bóng đá hàng đầu Việt Nam với sự tham gia của các đội
              bóng mạnh nhất cả nước.
            </p>
                    ...
                    <CardContent className="p-0 flex flex-col sm:flex-row">
                      <div className="flex-1 flex items-center justify-between p-4 bg-gradient-to-r from-background to-muted/20">
                        <div className="flex items-center gap-3 w-1/3">
                          <div className="font-bold text-base truncate text-right w-full">
                            {m.clbnha}
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-3 w-1/3 justify-end">
                          <div className="font-bold text-base truncate text-left w-full">
                            {m.clbkhach}
                          </div>
                        </div>
                      </div>
                  ...
                  <Card key={m.matran} className="border-l-4 border-l-blue-500">
                    <CardContent className="p-4">
                      ...
                      <div className="flex items-center gap-2 text-xs text-muted-foreground mt-2 border-t pt-2">
                        ...
                        {new Date(m.ngaythi_dau).toLocaleDateString("vi-VN")}
                        <span className="mx-1">•</span>
                        <MapPin className="w-3 h-3" />{" "}
                        {m.sanvandong || "Sân nhà"}
                      </div>
                    </CardContent>
                  </Card>
                ))
            ...
            <CardHeader className="flex flex-row items-center justify-between border-b pb-4">
              ...
              <Link to="/league-table">
                <Button variant="ghost" size="sm" className="gap-1">
                  Xem đầy đủ <ChevronRight className="w-4 h-4" />
                </Button>
              </Link>
            </CardHeader>
                  ...
                  <tbody>
                    {standings.length > 0 ? (
                      standings.map((team: any, idx: number) => {
                        ...
                        const isValidNumber = !Number.isNaN(hieuSo)
                        ...
                        return (
                          ...
                          >
                            <td className="px-4 py-3 text-center font-bold">
                              {idx === 0 ? (
                                <span className="w-6 h-6 rounded-full bg-yellow-100 text-yellow-700 dark:bg-yellow-900 dark:text-yellow-100 flex items-center justify-center mx-auto">
                                  1
                                </span>
                              ) : (
                                idx + 1
                            ...
                            <td className="px-4 py-3 font-medium">
                              <div className="flex items-center gap-2">
                                ...
                                {idx < 1 && (
                                  <span className="text-[10px] bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-100 px-1 rounded">
                                    ACL
                                  </span>
                                )}
                              </div>
              ...
              {topScorers.map((p: any, idx: number) => (
                <div key={idx} className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${idx === 0 ? "bg-yellow-100 text-yellow-700 dark:bg-yellow-900 dark:text-yellow-100" : "bg-gray-100 text-gray-600 dark:bg-muted dark:text-muted-foreground"}`}
                    >
                      {idx + 1}
                    </div>
                    <div>
