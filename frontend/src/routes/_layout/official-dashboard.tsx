...
function OfficialDashboard() {
  ...
  const { data: matchesData, isLoading } = useQuery({
    ...
    queryFn: () =>
      MatchesService.readMatches({ muagiai: currentSeason, limit: 100 }),
  })
          ...
          <div className="flex items-center gap-4 mb-2">
            ...
            <div>
              ...
              <p className="opacity-90">
                Chào mừng {currentUser?.ho_ten || "Trọng tài"} - Quan chức trận
                đấu
              </p>
            </div>
          </div>
      ...
      <div className="max-w-7xl mx-auto p-6 space-y-6">
        ...
        <div className="animate-in fade-in duration-300">
          ...
          {activeTab === "assignments" && (
            <AssignmentsTab matches={stats.list} onReport={handleStartReport} />
          )}
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
        <h3 className="font-bold text-lg text-gray-800 mb-4 flex items-center gap-2">
          <CalendarDays className="w-5 h-5 text-purple-600" /> Lịch trình sắp
          tới
        </h3>
        ...
        {upcomingMatch ? (
            ...
            <div className="flex justify-between items-start mb-4">
              <div>
                ...
                <h4 className="text-xl font-bold text-gray-900">
                  {upcomingMatch.doi_nha?.tenclb}{" "}
                  <span className="text-gray-400 mx-2">vs</span>{" "}
                  {upcomingMatch.doi_khach?.tenclb}
                </h4>
                <div className="flex items-center gap-2 text-gray-500 mt-2 text-sm">
                  <Clock className="w-4 h-4" />
                  {new Date(upcomingMatch.thoigianthidau).toLocaleString(
                    "vi-VN",
                  )}
                </div>
          ...
          <div className="text-center py-10 text-gray-500">
            Hiện không có lịch phân công sắp tới.
          </div>
        )}
      </div>
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
        <h3 className="font-bold text-lg text-gray-800 mb-4 flex items-center gap-2">
          <CheckCircle className="w-5 h-5 text-green-600" /> Báo cáo đã hoàn
          thành
        </h3>
              ...
              >
                <div>
                  <div className="font-bold text-sm">
                    {match.doi_nha?.tenclb} {match.tiso}{" "}
                    {match.doi_khach?.tenclb}
                  </div>
                  <div className="text-xs text-gray-500">
                    Vòng {match.vong} •{" "}
                    {new Date(match.thoigianthidau).toLocaleDateString()}
                  </div>
                </div>
        ...
        {matches.length === 0 ? (
          ...
          matches.map((m: any) => {
            ...
            return (
              <div
                key={m.matran}
                className="p-5 flex flex-col md:flex-row items-center justify-between hover:bg-gray-50 transition-colors gap-4"
              >
                <div className="flex-1">
                  ...
                  <div className="text-sm text-gray-500 flex items-center gap-1 mt-1">
                    ...
                    {m.san_nha?.tensan || m.masanvandong}
                  </div>
                </div>
...
function ReportFormTab({ matchId, matches, onCancel }: any) {
  // const _queryClient = useQueryClient() // unused for now, keep for future cache invalidation
  const match = matches.find((m: any) => m.matran === matchId)

  // BM5.1 - 5.3: Reporting Form Data
  const [score, setScore] = useState({ home: 0, away: 0 })
  const [events, _setEvents] = useState<any[]>([]) // Bàn thắng, thẻ phạt, thay người
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      console.log("Submitting report (BM5.1, 5.2, 5.3):", {
        matchId,
        score,
        events,
        stats,
    ...
    return (
      <div className="bg-white p-10 rounded-xl shadow-sm text-center border border-dashed">
        ...
        <p className="text-gray-500">
          Vui lòng chọn một trận đấu từ tab "Trận được phân công" để viết báo
          cáo.
        </p>
      </div>
    )
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
      <div className="px-6 py-4 border-b bg-purple-50 flex justify-between items-center">
        ...
        <span className="text-sm text-purple-600 font-medium bg-purple-100 px-3 py-1 rounded-full">
          {match.doi_nha?.tenclb} vs {match.doi_khach?.tenclb} (Vòng{" "}
          {match.vong})
        </span>
      </div>
        ...
        <div>
          ...
          <div className="flex items-center justify-between gap-8 p-6 bg-gray-50 rounded-xl border">
            <div className="flex-1 text-center">
              <Label
                htmlFor="home-score"
                className="block font-bold text-xl mb-2 text-gray-700"
              >
                {match.doi_nha?.tenclb}
              </Label>
              <Input
                id="home-score"
                type="number"
              ...
              <div className="flex items-center gap-2">
                <Input
                  ...
                  onChange={(e) =>
                    setStats({
                      ...stats,
                      homePossession: parseInt(e.target.value, 10),
                      awayPossession: 100 - parseInt(e.target.value, 10),
                    })
                  }
                />
              ...
              <div className="flex items-center gap-2">
                <Input
                  ...
                  onChange={(e) =>
                    setStats({
                      ...stats,
                      homeShots: parseInt(e.target.value, 10),
                    })
                  }
                />
                <span className="text-gray-400 text-xs">vs</span>
                <Input
                  ...
                  onChange={(e) =>
                    setStats({
                      ...stats,
                      awayShots: parseInt(e.target.value, 10),
                    })
                  }
                />
              </div>
              ...
              <div className="flex items-center gap-2">
                <Input
                  ...
                  onChange={(e) =>
                    setStats({
                      ...stats,
                      homeCorners: parseInt(e.target.value, 10),
                    })
                  }
                />
                <span className="text-gray-400 text-xs">vs</span>
                <Input
                  ...
                  onChange={(e) =>
                    setStats({
                      ...stats,
                      awayCorners: parseInt(e.target.value, 10),
                    })
                  }
                />
              </div>
          ...
          <div>
            <div className="flex justify-between items-center mb-3">
              ...
              <Button size="sm" variant="outline" className="h-7 text-xs">
                <Plus className="w-3 h-3 mr-1" /> Thêm sự kiện
              </Button>
            </div>
          ...
          <div className="space-y-4">
            <div>
              <Label className="mb-2 block font-bold text-sm">
                Cầu thủ xuất sắc nhất trận (MVP)
              </Label>
            ...
            <div>
              <Label className="mb-2 block font-bold text-sm">
                Ghi chú giám sát (Khán giả, an ninh...)
              </Label>
              <Textarea
...
function StatCard({ title, value, icon: Icon, color }: any) {
  return (
    <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex flex-col items-center justify-center text-center hover:shadow-md transition-shadow">
      <div
        className={`p-3 rounded-full mb-3 bg-opacity-10 ${color.replace("text-", "bg-")}`}
      >
