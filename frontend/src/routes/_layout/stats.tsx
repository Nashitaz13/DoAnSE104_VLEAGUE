...
import { Filter, Loader2, Trophy, AlertTriangle, Users } from "lucide-react"
import {
...
function StatisticsPage() {
  ...
  const seasonList = useMemo(() => {
    ...
    // Ensure 2025-2026 is in the list if not present
    const has2025 = list.find((s: any) => s.muagiai === "2025-2026");
    const finalList = has2025 ? list : [...list, { muagiai: "2025-2026" }];
  ...
  return (
    <div className="container mx-auto p-6 space-y-6 max-w-6xl">
      ...
      <Tabs defaultValue="scorers" className="w-full space-y-6">
        <TabsList className="grid w-full grid-cols-2 lg:grid-cols-3 h-auto p-1 bg-muted/50 rounded-xl">
