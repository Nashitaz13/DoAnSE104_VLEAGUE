...
function ManagerDashboard() {
  ...
  const myClub = useMemo(() => {
    let list = Array.isArray(allClubsData)
      ? allClubsData
      : (allClubsData as any)?.data || []
    const backupList = Array.isArray(backupClubsData)
      ? backupClubsData
      : (backupClubsData as any)?.data || []
    ...
    return list.find((club: any) => {
      ...
      if (
        username === "viettel" &&
        (clubId.includes("viettel") || clubId.includes("thecong"))
      )
        return true
      if (
        username === "hcmc" &&
        (clubId.includes("tphcm") || clubId.includes("hcm"))
      )
        return true
  ...
  const resolvedStadiumName = useMemo(() => {
    ...
    let list = Array.isArray(stadiumsData)
      ? stadiumsData
      : (stadiumsData as any)?.data || []
    const backupList = Array.isArray(backupStadiumsData)
      ? backupStadiumsData
      : (backupStadiumsData as any)?.data || []
    if (list.length === 0) list = backupList
    const foundStadium = list.find(
      (s: any) => s.masanvandong === stadiumId || s.MaSanVanDong === stadiumId,
    )
    const fullName =
      foundStadium?.tensanvandong || foundStadium?.tensan || stadiumId
    return String(fullName).replace("Sân vận động", "SVĐ")
  }, [myClub, stadiumsData, backupStadiumsData])

  if (loadingClubs && !myClub)
    return (
    ...
    <div className="min-h-[calc(100vh-4rem)] bg-background font-sans pb-10">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div className="bg-white/10 p-2 rounded-lg backdrop-blur-sm border border-white/20">
            <label
              ...
              className="text-xs font-bold text-blue-100 uppercase mb-1 block"
            >
              Mùa giải hoạt động
            </label>
            <Select value={selectedSeason} onValueChange={setSelectedSeason}>
              <SelectTrigger
                id="season-select"
                className="w-[180px] bg-white text-blue-900 dark:bg-neutral-800 dark:text-blue-100 border-none font-bold h-9"
              >
      ...
      <div className="max-w-7xl mx-auto p-6 space-y-6">
          ...
          {TABS.map((tab) => (
            <button
              ...
              className={`flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-medium transition-all ${
                activeTab === tab.id
                  ? "bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 shadow-sm font-bold"
        ...
        <div className="animate-in fade-in duration-300">
          ...
          {activeTab === "squad" && (
            <SquadTab clubId={myClub.maclb} season={selectedSeason} />
          )}
          {activeTab === "matches" && (
            <MatchesTab clubId={myClub.maclb} season={selectedSeason} />
          )}
        </div>
      </div>
    </div>
      ...
      <div className="bg-white dark:bg-card rounded-xl shadow-xl w-full max-w-lg overflow-hidden border border-border">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setFormData({ ...formData, tencauthu: e.target.value })
                }
                placeholder="Nguyễn Văn A"
              />
            </div>
            <div>
              <Label htmlFor="reg-number">Số áo</Label>
              <Input
                id="reg-number"
                type="number"
                value={formData.soaothidau}
                onChange={(e) =>
                  setFormData({ ...formData, soaothidau: e.target.value })
                }
              />
            </div>
            <div>
              <Label htmlFor="reg-position">Vị trí</Label>
              <select
                id="reg-position"
                ...
                value={formData.vitrithidau}
                onChange={(e) =>
                  setFormData({ ...formData, vitrithidau: e.target.value })
                }
              >
            ...
            <div>
              <Label htmlFor="reg-nationality">Quốc tịch</Label>
              <Input
                id="reg-nationality"
            ...
            <div>
              <Label htmlFor="reg-height">Chiều cao (cm)</Label>
              <Input
                id="reg-height"
                type="number"
                value={formData.chieucao}
                onChange={(e) =>
                  setFormData({ ...formData, chieucao: e.target.value })
                }
              />
            </div>
            <div>
              <Label htmlFor="reg-weight">Cân nặng (kg)</Label>
              <Input
                id="reg-weight"
                type="number"
                value={formData.cannang}
                onChange={(e) =>
                  setFormData({ ...formData, cannang: e.target.value })
                }
              />
            </div>
          </div>
        ...
        <div className="px-6 py-4 border-t bg-muted/50 flex justify-end gap-2">
          ...
          <Button
            ...
            disabled={mutation.isPending || !formData.tencauthu}
          >
    ...
    if (player) {
      setFormData({
        ...
        ngaysinh: player.ngaysinh
          ? new Date(player.ngaysinh).toISOString().split("T")[0]
          : "",
  ...
  const mutation = useMutation({
    ...
    onSuccess: (/* savedPlayer: any */) => {
      queryClient.setQueryData(["roster", clubId, season], (oldData: any) => {
        const currentList = Array.isArray(oldData)
          ? oldData
          : oldData?.data || []
      ...
      <div className="bg-white dark:bg-card rounded-xl shadow-xl w-full max-w-lg overflow-hidden border border-orange-200 dark:border-orange-800">
        ...
        <div className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <Label htmlFor="edit-number">Số áo</Label>
              <Input
                id="edit-number"
                type="number"
                value={formData.soaothidau}
...
function OverviewTab({
  ...
  const listStandings = Array.isArray(standings)
    ? standings
    : (standings as any)?.standings || []
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <StatCard
          title="Vị trí BXH"
          value={myRank?.hang ? `#${myRank.hang}` : "Chưa xếp"}
          icon={Target}
      ...
      <div className="bg-white dark:bg-card rounded-xl shadow-sm border border-border overflow-hidden">
        <div className="px-6 py-4 border-b border-border bg-muted/50 flex justify-between items-center">
          <h3 className="font-bold text-foreground flex items-center gap-2">
            <User className="w-5 h-5 text-muted-foreground" /> Hồ sơ đăng ký
          </h3>
        ...
        <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-12">
          <div>
            <div className="text-xs text-muted-foreground font-bold uppercase tracking-wider block mb-1">
              Tên Câu Lạc Bộ
            </div>
          ...
          <div>
            ...
            <div className="font-mono text-muted-foreground bg-muted inline-block px-2 py-0.5 rounded">
              {myClub.maclb}
            </div>
          </div>
          <div>
            ...
            <div className="font-medium text-foreground">{stadiumName}</div>
          </div>
          <div>
            <div className="text-xs text-muted-foreground font-bold uppercase tracking-wider block mb-1">
              Địa chỉ
            ...
            <div className="text-foreground/80">
              {myClub.san_nha?.diachi || "Chưa cập nhật"}
            </div>
          </div>
        </div>
      </div>
...
function SquadTab({ clubId, season }: { clubId: string; season: string }) {
  ...
  const stats = useMemo(() => {
    ...
    const avgAge = list.length
      ? (
          list.reduce((acc: number, p: any) => {
            const age = p.ngaysinh
              ? new Date().getFullYear() - new Date(p.ngaysinh).getFullYear()
              : 0
            return acc + age
          }, 0) / list.length
        ).toFixed(1)
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          ...
          <div className="text-xs text-muted-foreground mt-1 flex gap-3">
            <span>
              Ngoại binh: <b>{stats.foreign}</b>
            </span>
            <span>•</span>
            <span>
              Tuổi TB: <b>{stats.avgAge}</b>
            </span>
          </div>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12 bg-muted/30 rounded border border-dashed border-border">
          <p className="text-muted-foreground font-medium">
            Chưa có cầu thủ nào được đăng ký cho mùa giải {season}.
          </p>
          {season !== "2025-2026" && (
            <p className="text-xs text-blue-500 mt-2">
              *Mẹo: Dữ liệu mẫu trong hệ thống chủ yếu nằm ở mùa giải{" "}
              <b>2025-2026</b>.
            </p>
          )}
        </div>
          ...
          <table className="w-full text-sm text-left">
            <thead className="bg-muted text-muted-foreground text-xs uppercase font-bold">
              <tr>
                ...
                <th className="px-4 py-3 text-center" title="Chiều cao (cm)">
                  Cao
                </th>
                <th className="px-4 py-3 text-center" title="Cân nặng (kg)">
                  Nặng
                </th>
            ...
            <tbody className="divide-y divide-border">
              {list.map((p: any) => {
                const birthYear = p.ngaysinh
                  ? new Date(p.ngaysinh).getFullYear()
                  : "-"
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`inline-block px-2 py-0.5 rounded text-xs font-bold w-10 text-center
      ...
      {isLoading ? (
        ...
        <div className="p-16 text-center">
          ...
          <p className="text-muted-foreground max-w-xs mx-auto mt-2">
            Không tìm thấy trận đấu nào cho mùa giải {season}. Hãy thử chuyển
            sang mùa giải <b>2024-2025</b>.
          </p>
        </div>
      ) : (
        <div className="divide-y divide-border">
          {sortedList.map((m: any) => {
            ...
            const matchDate = m.thoigianthidau
              ? new Date(m.thoigianthidau).toLocaleDateString("vi-VN")
              : "Chưa xếp lịch"
            return (
              <div
                key={m.matran}
                className="flex flex-col md:flex-row items-center justify-between p-4 hover:bg-muted/50 transition-colors gap-4"
              >
                <div className="flex items-center gap-3 w-full md:w-auto">
                  <div className="bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 px-3 py-1 rounded text-xs font-bold uppercase min-w-[80px] text-center">
                    Vòng {m.vong}
                  </div>
                ...
                <div className="flex-1 flex items-center justify-center gap-4 w-full">
                  <span
                    className={`text-right flex-1 font-bold ${isHome ? "text-blue-700 dark:text-blue-400" : "text-foreground"}`}
                  >
                    {m.doi_nha?.tenclb || m.maclb_nha}
                  </span>
                  <div className="bg-gray-900 dark:bg-gray-800 text-white px-4 py-1.5 rounded-lg font-mono font-bold text-lg shadow-sm min-w-[80px] text-center">
                    {m.tiso || "vs"}
                  </div>
                  <span
                    className={`text-left flex-1 font-bold ${!isHome ? "text-blue-700 dark:text-blue-400" : "text-foreground"}`}
                  >
                    {m.doi_khach?.tenclb || m.maclb_khach}
                  </span>
                </div>
                <div className="w-full md:w-auto text-center md:text-right">
                  <span
                    className={`text-xs px-2 py-1 rounded border ${isHome ? "bg-green-50 dark:bg-green-900/30 border-green-200 dark:border-green-800 text-green-700 dark:text-green-300" : "bg-orange-50 dark:bg-orange-900/30 border-orange-200 dark:border-orange-800 text-orange-700 dark:text-orange-300"}`}
                  >
...
function StatCard({ title, value, icon: Icon, color }: any) {
  return (
    <div className="bg-white dark:bg-card p-5 rounded-xl shadow-sm border border-border flex items-center justify-between hover:shadow-md transition-shadow">
      <div>
        <div
          className={`text-2xl font-bold ${color} truncate max-w-[180px]`}
          title={value}
        >
          {value}
        </div>
        <div className="text-xs font-bold uppercase text-muted-foreground mt-1 tracking-wider">
          {title}
        </div>
      </div>
      <div
        className={`p-3 rounded-full ${color.replace("text-", "bg-")} bg-opacity-10 dark:bg-opacity-20`}
      >
