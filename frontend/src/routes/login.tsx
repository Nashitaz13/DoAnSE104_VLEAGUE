...
function LoginPage() {
  ...
  const inferRoleFromUser = (user: any): string => {
    ...
    if (username.includes("admin") || username.includes("quantri"))
      return "admin"
    if (username.includes("btc")) return "btc"
    if (username.includes("trongtai") || username.includes("referee"))
      return "referee"
    if (
      username.includes("clb") ||
      username.includes("club") ||
      username.includes("quanly")
    )
      return "club"
      ...
      <div className="w-full max-w-md space-y-8">
        ...
        <div className="bg-white dark:bg-card py-8 px-4 shadow-xl sm:rounded-xl sm:px-10 border dark:border-border transition-colors duration-300">
          <form className="space-y-6" onSubmit={handleSubmit(onSubmit)}>
              ...
              <Select value={uiRole} onValueChange={setUiRole}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="btc" className="font-medium">
                    <div className="flex items-center gap-2">
                      <Briefcase className="w-4 h-4 text-red-600" /> Ban Tổ Chức
                      (BTC)
                    </div>
                  </SelectItem>
                  <SelectItem value="club" className="font-medium">
                    <div className="flex items-center gap-2">
                      <UserCircle className="w-4 h-4 text-blue-600" /> Đội bóng
                      / CLB
                    </div>
                  </SelectItem>
                  <SelectItem value="referee" className="font-medium">
                    <div className="flex items-center gap-2">
                      <Flag className="w-4 h-4 text-yellow-600" /> Trọng tài /
                      Giám sát
                    </div>
                  </SelectItem>
                </SelectContent>
            ...
            <div className="space-y-2">
              <Label htmlFor="username" className="dark:text-foreground">
                Tên đăng nhập
              </Label>
              <Input
                id="username"
                className="h-11 bg-white dark:bg-background dark:border-input dark:text-foreground"
                {...register("username")}
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <Input
                  ...
                  className="h-11 pr-10 bg-white dark:bg-background dark:border-input dark:text-foreground"
                  {...register("password")}
            ...
            <Button
              type="submit"
              className="w-full h-11 bg-red-700 hover:bg-red-800 text-white font-bold dark:bg-red-800 dark:hover:bg-red-900"
              disabled={isLoading}
            >
              {isLoading ? (
                <>
          ...
          <div className="mt-6 text-center text-sm">
            ...
            <Link
              to="/register"
              className="font-bold text-red-700 hover:underline dark:text-red-500"
            >
              Đăng ký ngay
            </Link>
          </div>
